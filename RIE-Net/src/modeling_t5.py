...
class JointEncoder(T5Stack):
    def __init__(self, config, embed_tokens=None):
        ...
        self.guanxi = nn.Parameter(torch.FloatTensor(3000, 768))
    ...
    def forward(
        ...
        output_text, hidden_text = self.GRU(inputs_embeds)
        encoded_output = hidden_text[-1].unsqueeze(1)
        encoded_output_expand = encoded_output.expand(-1, vis_embeds.size(1), -1)
        similar = torch.cosine_similarity(encoded_output_expand, vis_embeds, dim=2)
        max_values, max_indices = torch.topk(similar, k=2, dim=1)
        iou_z = boxes[:, :, 0]

        box1 = boxes[:,0,:]
        for i in range(boxes.shape[0]):
            box1[i,0] = torch.min(torch.min(torch.min(boxes[i,max_indices[i, 0], 1],boxes[i,max_indices[i, 1], 1]),boxes[i,max_indices[i, 0], 0]),boxes[i,max_indices[i, 1], 0])
            box1[i,1] = torch.max(torch.max(torch.max(boxes[i,max_indices[i, 0], 1],boxes[i,max_indices[i, 1], 1]),boxes[i,max_indices[i, 0], 0]),boxes[i,max_indices[i, 1], 0])
            box1[i,2] = torch.min(torch.min(torch.min(boxes[i,max_indices[i, 0], 3],boxes[i,max_indices[i, 1], 3]),boxes[i,max_indices[i, 0], 2]),boxes[i,max_indices[i, 1], 2])
            box1[i,3] = torch.max(torch.max(torch.max(boxes[i,max_indices[i, 0], 3],boxes[i,max_indices[i, 1], 3]),boxes[i,max_indices[i, 0], 2]),boxes[i,max_indices[i, 1], 2])

        for j in range(boxes.shape[1]):
            box2_zuo = torch.min(boxes[:, j, 0],boxes[:, j, 1])
            box2_you = torch.max(boxes[:, j, 0],boxes[:, j, 1])
            box2_xia = torch.min(boxes[:, j, 2],boxes[:, j, 3])
            box2_shang = torch.max(boxes[:, j, 2],boxes[:, j, 3])
            box2 = torch.stack((box2_zuo, box2_you, box2_xia, box2_shang), dim=1)
        ...
        max_values_v, max_indices_v = torch.topk(iou_z, k=1, dim=1)
 
        vis_scene = vis_embeds[:,0,:]

        
        for i in range(max_indices_v.shape[0]):
            vis_scene[i] = vis_embeds[i,max_indices_v[i]]
        vis_scene = vis_scene.unsqueeze(1)

        vis_scene = vis_scene#.expand(-1, 2, -1)

        mask1 = (objects_ids >= 0) & (objects_ids < 20)
        objects_ids[mask1] = objects_ids[mask1] // 2
        mask2 = (objects_ids >= 20) & (objects_ids < 1000)
        objects_ids[mask2] = objects_ids[mask2] // 20 + 9
        mask3 = (objects_ids >= 1000) & (objects_ids < 10000)
        objects_ids[mask3] = objects_ids[mask3] // 50 + 39
        mask4 = (objects_ids >= 10000) & (objects_ids < 20000)
        objects_ids[mask4] = objects_ids[mask4] // 200 + 189
        mask5 = (objects_ids >= 20000) & (objects_ids < 31000)
        objects_ids[mask5] = objects_ids[mask5] // 1000 + 269
        mask6 = objects_ids >= 31000
        objects_ids[mask6] = 299
        objects_ids = objects_ids.to(torch.int64)
        one_hot = F.one_hot(objects_ids, num_classes=300)
        few_shot = one_hot.reshape(one_hot.shape[0], one_hot.shape[1] * one_hot.shape[2])
        few_shot = few_shot.float()
        zuihou = torch.matmul(few_shot, self.guanxi)
        objects_relation_q = zuihou.unsqueeze(1)#.expand(-1,2,-1)
        objects_relation = torch.cat((objects_relation_q, objects_relation_q, objects_relation_q, objects_relation_q), dim=0)
        for i in range(objects_relation_q.shape[0]):
            objects_relation[4*i:4*i+4] = objects_relation_q[i]
        V_L = vis_embeds.size(1)
        object_L = objects_relation.size(1)
        scene_L = vis_scene.size(1)
        inputs_embeds = torch.cat([inputs_embeds, vis_embeds, vis_scene], dim=1)
        ...
        if vis_attention_mask is None:
            vis_attention_mask = attention_mask.new_ones(B, V_L+ scene_L)    #
        ...
        hidden_states = torch.cat((hidden_states, objects_relation), dim=1)
