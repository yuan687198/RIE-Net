...
class VCRFineTuneDataset(Dataset):
    def __init__(self, split='train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train'):
        ...
        if 't5' in self.args.backbone:
            if self.args.use_vision:
                self.tokenizer = VLT5TokenizerFast.from_pretrained('/data/yuanmq/VLT5/t5-base',
                    #args.backbone,
                    max_length=self.args.max_text_length,
                    do_lower_case=self.args.do_lower_case)
            else:
                self.tokenizer = T5TokenizerFast.from_pretrained('/data/yuanmq/VLT5/t5-base',
                    #args.backbone,
                    max_length=self.args.max_text_length,
                    do_lower_case=self.args.do_lower_case)
    ...
    def __getitem__(self, idx):
        ...
        out_dict['objects'] = datum['objects']
        ...
        for i in range(len(objects)):
            ...
            for j in range(len(objects[i])):
                if j != 0:
                    yy = objects[i][j]
                    zh = zh + str(' ') + str(yy)
            zz = self.tokenizer(zh, padding=True, truncation=True, max_length=args.max_text_length, return_tensors='pt').input_ids
            objects_.append(zh)
            objects_ids.append(zz)
        batch_entry['objects_ids'] = objects_ids
        batch_entry['objects_'] = objects_
