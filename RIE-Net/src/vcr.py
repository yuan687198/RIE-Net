...
class Trainer(TrainerBase):
    def __init__(self, args, train_loader=None, val_loader=None, test_loader=None, train=True):
        ...
        self.args.log_train_accuracy = False
    ...
    def train(self):
        ...
        global_step = 0
        #self.args.epochs = 20
        for epoch in range(self.args.epochs):     # self.args.epochs
            ...
            for step_i, batch in enumerate(self.train_loader):
                ...
                args.log_train_accuracy = False
                batch['log_train_accuracy'] = self.args.log_train_accuracy    #self.args.log_train_accuracy
                
                #print(args.fp16)
                #print(batch)

                if self.args.fp16 and _use_native_amp:     #self.args.fp16
                    with autocast():
                        #if self.args.distributed:
                        #    results = self.model.module.train_step(batch)
                        #else:
                            results = self.model.train_step(batch)      #self.model.train_step(batch)
                else:
                    #if self.args.distributed:
                    #    results = self.model.module.train_step(batch)
                    #else:
                ...
                # Update Parameters
                if self.args.clip_grad_norm > 0:      #  if self.args.clip_grad_norm > 0:
                    if self.args.fp16 and _use_native_amp:   # if self.args.fp16 and _use_native_amp:
                        self.scaler.unscale_(self.optim)
                        torch.nn.utils.clip_grad_norm_(
                            self.model.parameters(), self.args.clip_grad_norm)      #self.args.clip_grad_norm
                    elif self.args.fp16 and _use_apex:    # if self.args.fp16 and _use_native_amp:
                        torch.nn.utils.clip_grad_norm_(amp.master_params(
                            self.optim), self.args.clip_grad_norm)       #self.args.clip_grad_norm
                    else:
                        torch.nn.utils.clip_grad_norm_(
                            self.model.parameters(), self.args.clip_grad_norm)    #self.args.clip_grad_norm
        ...
        if self.verbose:
            ...
            #print(self.args.output)   #hou
            #print(self.evaluate_test(self.test_loader, dump_path=dump_path))  #hou
...
if __name__ == "__main__":
    ...
    print('T5-base')
    model = T5ForConditionalGeneration.from_pretrained('/data/yuanmq/VLT5/t5-base')    #t5-base
    tokenizer = T5Tokenizer.from_pretrained('/data/yuanmq/VLT5/t5-base')      #t5-base
    print('Done!')
